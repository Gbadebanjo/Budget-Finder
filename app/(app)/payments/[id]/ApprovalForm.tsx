...
export default function ApprovalForm({ paymentId }: { paymentId: string }) {
  ...
  async function handleAction(action: 'approved' | 'rejected') {
    ...
    if (!user) { setError('Not authenticated'); setLoading(null); return }
    ...
    if (logErr) { setError(logErr.message); setLoading(null); return }
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-6 space-y-4">
      <h2 className="font-semibold text-tx">Review Payment</h2>
      ...
      <div className="space-y-1.5">
        <label className="block text-xs font-semibold text-muted uppercase tracking-wide">
          Comment <span className="text-subtle font-normal normal-case">(optional)</span>
        </label>
        <textarea
          rows={3}
          value={comment}
