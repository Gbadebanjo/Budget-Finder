...
export default async function DashboardPage() {
  ...
  // Pending approvals: payments needing review that were NOT submitted by this user
  const { data: pendingPayments } = isPrivileged
    ? await supabase
        .from('payments')
  ...
  const stats = [
    ...
    { label: 'Pending',  value: pending  ?? 0, color: 'text-yellow-600 dark:text-yellow-400'     },
    { label: 'Approved', value: approved ?? 0, color: 'text-green-600  dark:text-green-400'      },
    { label: 'Rejected', value: rejected ?? 0, color: 'text-red-600    dark:text-red-400'        },
  ]
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-tx">
            {isAdmin ? 'Admin Dashboard' : isPrivileged ? 'Dashboard' : 'Dashboard'}
          </h1>
          <p className="text-muted text-sm mt-1">
            Welcome back,{' '}
            <span className="text-tx font-medium">{profile?.full_name || profile?.email}</span>
          </p>
        </div>
        <Link
          href="/payments/new"
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted mt-0.5">Send an invitation email to onboard a new team member.</p>
            </div>
          </div>
          <InviteForm />
        </section>
      ...
      {isPrivileged && (
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Link href="/payments?status=pending" className="text-sm text-accent hover:text-accent-hover font-medium transition-colors">
              View all →
            </Link>
          </div>
          <PendingApprovals
      ...
      <section>
        <div className="bg-surface border border-border rounded-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            ...
            <Link href="/payments" className="text-sm text-accent hover:text-accent-hover font-medium transition-colors">
              View all →
            </Link>
          </div>
          <div className="divide-y divide-border-soft">
            {(recent ?? []).length === 0 && (
              <div className="px-6 py-12 text-center">
                ...
                <Link href="/payments/new" className="text-accent text-sm font-medium hover:underline mt-1 inline-block">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted mt-0.5">
                    {isPrivileged && p.creator && (
                      <span className="font-medium">{p.creator.full_name || p.creator.email} · </span>
                    )}
